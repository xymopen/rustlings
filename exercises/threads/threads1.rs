...
fn main() {
    ...
    thread::spawn(move || {
        for _ in 0..10 {
            ...
            // I am confusing about `.deref()` and `*` operator here
            // `.deref()` should be called automatically
            status_shared.deref().lock().unwrap().jobs_completed += 1;
        }
    });
    while status.deref().lock().unwrap().jobs_completed < 10 {
        println!("waiting... ");
